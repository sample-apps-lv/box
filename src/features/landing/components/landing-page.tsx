...
const LandingPage = () => {
  ...
  return (
    <div className="flex-1 flex flex-col items-start justify-center px-8 py-16 max-w-4xl">
      {/* System status */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <span className={`w-2 h-2 ${isConnected ? 'bg-hud-green' : 'bg-hud-red'}`} />
      ...
      >
        Upload a BOM. Watch AI agents analyse every component in real-time. Get supply chain risk, IPC compliance, version upgrades, and a go/no-go recommendation — all in one stream.
      </motion.p>
      ...
      >
        <Link to="/analyse">
          <Button variant="default" size="lg" className="font-mono text-hud-data uppercase tracking-[0.15em] bg-primary text-primary-foreground hover:bg-primary/80 gap-2 px-8 h-12">
        ...
        <Link to="/dashboard">
          <Button variant="outline" size="lg" className="font-mono text-hud-data uppercase tracking-[0.15em] border-primary/30 text-primary hover:bg-primary/10 h-12">
            View Dashboard
          </Button>
        </Link>
      </motion.div>
      ...
      >
        {[
          ...
          <div key={feat.label} className="hud-glass border-l-2 border-l-primary/20 p-4">
            <feat.icon className="w-4 h-4 text-primary mb-2" />
            <div className="font-mono text-hud-label text-primary tracking-[0.15em] mb-1">{feat.label}</div>
            <div className="text-hud-data text-muted-foreground">{feat.desc}</div>
          </div>
        ))}
      </motion.div>
    </div>
